...
const Appearance = () => {
  const [activeTheme, setActiveTheme] = useState(
    localStorage.getItem("portal-theme") || "neon-blue",
  );
  const [fontSize, setFontSize] = useState(
    localStorage.getItem("portal-font-size") || "medium",
  );
  const [activeFont, setActiveFont] = useState(
    localStorage.getItem("portal-font-family") || "sans",
  );
  ...
  const handleRestore = () => {
    Swal.fire({
      title: getTranslation("appearance_restore_title"),
      text: getTranslation("appearance_restore_text"),
      icon: "warning",
      showCancelButton: true,
      confirmButtonText: getTranslation("appearance_restore_confirm"),
  ...
  const handleFontChange = (fontId) => {
    ...
    Swal.fire({
      ...
      position: "top-end",
      icon: "success",
      title: getTranslation("appearance_font_success"),
  ...
  const handleModeChange = (newMode) => {
    ...
    Swal.fire({
      ...
      position: "top-end",
      icon: "success",
      title: `${newMode === "dark" ? getTranslation("appearance_mode_success_prefix") : getTranslation("appearance_mode_success_suffix")} ${getTranslation("appearance_mode_active")}`,
      showConfirmButton: false,
      timer: 1500,
  ...
  const handleThemeChange = (themeId) => {
    ...
    Swal.fire({
      ...
      position: "top-end",
      icon: "success",
      title: `${selectedTheme.name} ${getTranslation("appearance_theme_success_prefix")}`,
      showConfirmButton: false,
      timer: 2000,
      background: "#0f172a",
      color: "#f8fafc",
  ...
  return (
    <div className="page-shell space-y-8 pb-12">
      <PageHeader
        ...
        subtitle={getTranslation("appearance_subtitle")}
      />
      ...
      <MDBRow className="g-6">
              ...
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 rounded-2xl bg-blue-500/10 flex items-center justify-center text-blue-400 border border-blue-500/20 shadow-sm">
                  ...
                  <h3 className="text-xl font-black text-white tracking-tight">
                    {getTranslation("appearance_theme_archetypes")}
                  </h3>
              </div>
            </div>
                ...
                {themes.map((theme) => (
                  ...
                  >
                    <div className="flex items-center gap-2 xs:gap-4">
                      <div
                        className="h-8 xs:h-12 md:h-14 w-8 xs:w-12 md:w-14 rounded-lg xs:rounded-2xl flex items-center justify-center text-white shadow-lg shrink-0"
                        style={{
                      ...
                      >
                        <MDBIcon
                          fas
                          icon="brush"
                          className="text-[10px] xs:text-base"
                        />
                      </div>
                      <div className="min-w-0">
                        <h4 className="text-[9px] xs:text-xs md:text-sm font-black text-white uppercase tracking-tighter truncate">
                          {theme.name}
                        </h4>
                        <p className="text-[7px] xs:text-[9px] md:text-[10px] text-slate-500 font-bold uppercase tracking-widest truncate">
                          {theme.desc}
                        </p>
                      </div>
                      {activeTheme === theme.id && (
                        <div className="ml-auto h-4 w-4 xs:h-6 xs:w-6 rounded-full bg-emerald-500 flex items-center justify-center shadow-lg shadow-emerald-500/20 shrink-0">
                          <MDBIcon
                            fas
                            icon="check"
                            className="text-white text-[7px] xs:text-[10px]"
                          />
                        </div>
                      )}
                    </div>
        ...
        <MDBCol lg="4">
          <div className="space-y-4 xs:space-y-6">
            ...
            >
              <p className="text-[9px] xs:text-[10px] font-black uppercase tracking-[0.2em] xs:tracking-[0.3em] text-slate-500 mb-4 xs:mb-8">
                {getTranslation("appearance_mode_sync")}
              </p>
              <div className="flex gap-2 xs:gap-4">
                ...
                >
                  ...
                  <span className="text-[8px] xs:text-[9px] font-black uppercase tracking-widest">
                    {getTranslation("appearance_solar")}
                  </span>
                </button>
                <button
                ...
                >
                  ...
                  <span className="text-[8px] xs:text-[9px] font-black uppercase tracking-widest">
                    {getTranslation("appearance_night")}
                  </span>
                </button>
              </div>
            </MotionDiv>
            ...
            >
              <p className="text-[9px] xs:text-[10px] font-black uppercase tracking-[0.2em] xs:tracking-[0.3em] text-slate-500 mb-4 xs:mb-8">
                {getTranslation("appearance_font_matrix")}
              </p>
              <div className="space-y-2 xs:space-y-3">
                {fonts.map((f) => (
                  ...
                  >
                    <span
                      className="text-[10px] xs:text-xs font-bold truncate pr-2"
                    ...
                    {activeFont === f.id && (
                      <MDBIcon
                        fas
                        icon="check-circle"
                        className="text-primary-theme shrink-0 text-xs"
                      />
                    )}
                  </button>
                ))}
            ...
            >
              <p className="text-[9px] xs:text-[10px] font-black uppercase tracking-[0.2em] xs:tracking-[0.3em] text-slate-500 mb-4 xs:mb-8">
                {getTranslation("appearance_typography_scaling")}
              </p>

              <div className="grid grid-cols-3 gap-2 xs:gap-3">
                {[
                  { id: "small", label: getTranslation("appearance_compact"), icon: "compress-alt" },
                  { id: "medium", label: getTranslation("appearance_standard"), icon: "font" },
                  { id: "large", label: getTranslation("appearance_expanded"), icon: "expand-alt" },
                ].map((size) => (
                  <button
                    key={size.id}
                  ...
                  >
                    <MDBIcon
                      fas
                      icon={size.icon}
                      className="text-[9px] xs:text-sm shrink-0"
                    />
                    <span className="text-[6px] xs:text-[8px] font-black uppercase tracking-widest text-center leading-tight">
                      {size.label}
                    </span>
                  </button>
                ))}
            ...
            <MDBCard className="surface-card rounded-[1.5rem] xs:rounded-[2rem] md:rounded-[2.5rem] p-4 xs:p-6 md:p-8 border border-white/5 shadow-2xl relative overflow-hidden group">
              ...
              <div className="relative z-10 text-center">
                <p className="text-[9px] xs:text-[10px] font-black uppercase tracking-[0.2em] xs:tracking-[0.3em] text-slate-500 mb-4 xs:mb-6">
                  {getTranslation("appearance_reset_protocols")}
                </p>
                <MDBBtn
                  onClick={handleRestore}
                  outline
                  className="btn-ui-glass w-full rounded-xl md:rounded-2xl py-2 xs:py-3 border-white/5 text-slate-400 hover:text-white transition-all uppercase text-[8px] xs:text-[10px] font-black tracking-widest"
                >
                  {getTranslation("appearance_factory_reboot")}
                </MDBBtn>
              </div>
            </MDBCard>
          </div>
