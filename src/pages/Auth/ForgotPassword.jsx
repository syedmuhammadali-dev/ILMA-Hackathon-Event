...
const ForgotPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!user) {
      Swal.fire({
        icon: "error",
        title: getTranslation("notfound_title"),
        text: getTranslation("auth_forgot_notfound"),
    ...
    Swal.fire({
      icon: "success",
      title: getTranslation("auth_reset_link_sent"),
      text: getTranslation("auth_reset_link_redirect"),
      timer: 2500,
        ...
        <div className="auth-glass-card rounded-3xl p-10 md:p-12">
          ...
          >
            <h1 className="text-3xl font-black text-white-force tracking-tight mb-2">
              {getTranslation("auth_forgot_heading")}
            </h1>
            <p
              className="text-sm leading-relaxed"
              style={{ color: "rgba(148,163,184,0.75)" }}
            >
              {getTranslation("auth_forgot_subtitle")}
            </p>
          </motion.div>
...
{loading ? (
                <>
                  <Spinner /> {getTranslation("auth_sending")}
                </>
              ) : (
                <>
                  {getTranslation("auth_send_reset_link")}
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="w-4 h-4"
                    fill="none"
